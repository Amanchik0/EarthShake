...
const EventEditPage: React.FC = () => {
  ...
  useEffect(() => {
    if (originalEvent && formData.coordinates.x && formData.coordinates.y) {
      console.log(' Данные события загружены, инициализируем карту');
  ...
  const initializeMap = () => {
    ...
    try {
      console.log(' Инициализируем карту с координатами:', formData.coordinates);
      ...
      markerRef.current.on('dragend', async () => {
        if (markerRef.current) {
          ...
          console.log(' Маркер перетащен:', lngLat);
